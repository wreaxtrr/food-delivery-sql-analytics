...
create table categories (category_id bigserial primary key, category_name varchar(100) not null, parent_category_id bigint references categories(category_id));
...
create table orders (
    ...
    order_status varchar(30) not null check (order_status in ('created', 'paid', 'cooking', 'delivering', 'delivered', 'cancelled')),
...
create table payments (
    ...
    payment_method varchar(30) not null check (payment_method in ('card', 'cash', 'apple_pay', 'google_pay')),
...
create table order_status_history (
    ...
    order_id bigint not null references orders(order_id),
    status varchar(30) not null check (status in ('created', 'paid', 'cooking', 'delivering', 'delivered', 'cancelled')),
...
create table app_events (
    ...
    event_type varchar(50) not null check (event_type in ('app_open', 'restaurant_view', 'product_view', 'add_to_cart', 'checkout_start', 'order_created')),
